            ...
            {files.length > 0 &&
              files.map((file, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center w-full mt-2 justify-between text-neutral-600 dark:text-neutral-400 text-xs sm:text-sm">
                    <motion.p
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      layout
      ...
      {Array.from({ length: rows }).map((_, row) =>
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              className={`w-6 sm:w-10 h-6 sm:h-10 flex flex-shrink-0 rounded-[2px] ${
                index % 2 === 0
